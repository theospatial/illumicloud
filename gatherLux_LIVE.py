#-------------------------------------------------------------------------------
# Name:        gatherLux_LIVE.py
# Purpose:     use arcpy module to take csv generated from live night data in 
...
source_live = ws + '\\LivePaths'
# location to put pointcloud shp and gdb
target_live = ws + '\\LivePoints'
# Location to put analysis results
result_live = ws + '\\LiveResults'
# shp filename
liveshp = "luxPoints_LIVE_N.shp"
...
for file in filelist:
    ...
    file_path = source_live + '\\' + file_name + ".csv"
...
arcpy.CreateFeatureclass_management(target_live+'\\', fc, "Point")
print 'shapefile created'

# use output ws
env.workspace = target_live
...
in_cursor = arcpy.da.InsertCursor(target_live + '\\' + fc, ["SHAPE@"])
#update cursor for fields
up_cursor = arcpy.da.UpdateCursor(target_live + '\\' + fc, ['date','time','lux'])
